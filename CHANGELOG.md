...
- **Schema v4: a layered link + a "Minimal link" option.** The share token is
  ...
  blob (`{ c: created, t: [titles] }`). A new **Minimal link** checkbox in the
  build view drops `ext` entirely: URLs only, roughly half the characters, at
  the cost of page titles and the "shared on <date>" line (the viewer already
  falls back to the site name). Ad / analytics query params (`utm_*`, `fbclid`,
  `gclid`, ~35 in all) are stripped from every URL on encode -- they never
  change which page loads. **Every existing link still opens** -- the decoder
  reads v1, v2, v3 and v4. A v4 link needs a viewer that also has the v4
  decoder.
