...
- **Picking a shortener no longer errors before you've configured it.** Choosing
  *Tab Share shortener* or *Custom endpoint* now just reveals its fields and
  ...
  enter an address.
- The viewer footer's **Ko-fi** entry is now the "Buy me a Beer" sticker (a
  bundled purple SVG) instead of a text link -- still a plain `<a href>`, still
  no network request until clicked.
